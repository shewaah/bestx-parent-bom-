...
public class TW_SendOrderEventHandler extends BaseOperationEventHandler {
   ...
   public TW_SendOrderEventHandler(Operation operation, MarketBuySideConnection twConnection, SerialNumberService serialNumberService, long waitingExecutionDelay, long orderCancelDelay,
         BestXConfigurationDao bestXConfigurationDao){
      super(operation);
            ...
            case CANCELLED:
               ...
               if (!operation.isCustomerRevokeReceived()) {
                  ...
                  if ((quotedDealers != null && marketOrderTarget != null && quotedDealers.toLowerCase().contains(marketOrderTarget.toLowerCase()))) {
                     ...
                     if (maxRetries == null) {
                        LOGGER.warn("Order {}, received a CANCEL exec report with reason [{}], cannot proceed with execution retries due to unconfigured or misconfigured maximum number of trials",
                              order.getFixOrderId(), text);
                        operation.setStateResilient(new TW_RejectedState(isNoDealerReply ? Messages.getString("TWNoReply.MaxRetriesNotConfigured")
                              : Messages.getString("TWDealerNotInQuotedList.MaxRetriesNotConfigured", marketOrderTarget, quotedDealers)), ErrorState.class);
                     }
                     else {
                        try {
                           Integer maxTries;
                           ...
                           if (actualTries < maxTries) {
                              ...
                              operation.setStateResilient(
                                    new TW_SendOrderState(
                                          isNoDealerReply ? Messages.getString("TWNoReply.Retry") : Messages.getString("TWDealerNotInQuotedList.Retry", marketOrderTarget, quotedDealers)),
                                    ErrorState.class);
                           }
                           else {
                              ...
                              marketExecutionReport.setReason(RejectReason.TRADER_REJECTED);
                              operation.setStateResilient(new TW_RejectedState(isNoDealerReply ? Messages.getString("TWNoReply.Rejected", maxRetries)
                                    : Messages.getString("TWDealerNotInQuotedList.Rejected", marketOrderTarget, quotedDealers, maxRetries)), ErrorState.class);
                           }
                        }
                  ...
                  if (isCancelBestXInitiative) {
                     ...
                     operation.setStateResilient(new TW_CancelledState("No answer received after the configurated number of seconds. Order has been automatically cancelled by BestX:FI-A"),
                           ErrorState.class);
                  }
                  else {
                     marketExecutionReport.setReason(RejectReason.TRADER_REJECTED);
   ...
   public void onTimerExpired(String jobName, String groupName) {
      ...
      if (jobName.equals(handlerJobName)) {
         ...
         try {
            ...
            operation.setStateResilient(new WarningState(operation.getState(), e, Messages.getString("TW_MarketRevokeOrderError", operation.getOrder().getFixOrderId())), ErrorState.class);
         }
      }
   ...
   public void onFixRevoke(CustomerConnection source) {
      ...
      //onFixRevoke may be invoked by customer and must cancel the order from BestX:FI-A (this should happen setting customerRevokeReceived to true)
