...
//				}
    ...
    public void sendExecutionReport(Operation source, Quote quote, Order order, String orderId, Attempt attempt, ExecutionReport executionReport) throws BestXException {
        ...
        if (executionReport instanceof CSPOBexExecutionReport && ExecutionReportState.NEW.equals(executionReport.getState())) {
        	...
        	bean = new OMS1FixExecutionReportOutputLazyBean(fixSessionId, quote, order, orderId, attempt, executionReport, micCodeService);
        }
        fixGatewayConnection.sendExecutionReport(this, bean);
    }
    ...
    public double getAvgInputPerSecond() {
        ...
        return (double) (statTotalFixOrderNotifications.get() + statTotalFixRfqNotifications.get() + statTotalFixQuotesAcknowledged.get() + statTotalFixQuotesNotAcknowledged.get() + statTotalFixTradesAcknowledged.get() + statTotalFixTradesNotAcknowledged.get())
                / (currentTime - startTime + 1) * 1000.0; // avoid div per zero
    }
    ...
    public double getAvgOutputPerSecond() {
        ...
        long currentTime = DateService.currentTimeMillis();
        long startTime = statStart.getTime();
        return (double) (statRfqResponses.get() + statOrderRejects.get() + statRfqReject.get() + statRejectExecutionReport.get() + statFillExecutionReport.get()) / (currentTime - startTime + 1) * 1000.0; // avoid
                                                                                                                                                                              // div
                                                                                                                                                                              // per
                                                                                                                                                                              // zero
    }
    ...
    public double getPctOfFailedInput() {
        long totInput = statTotalFixOrderNotifications.get() + statTotalFixRfqNotifications.get() + statTotalFixQuotesAcknowledged.get() + statTotalFixQuotesNotAcknowledged.get() + statTotalFixTradesAcknowledged.get()
                + statTotalFixTradesNotAcknowledged.get();
        if (totInput > 0) {
            return (double) (statFailedFixOrderNotifications.get() + statFailedFixRfqNotifications.get() + statFailedFixTradesAcknowledged.get() + statFailedFixTradesNotAcknowledged.get() + statFailedFixQuotesAcknowledged.get() + statFailedFixQuotesNotAcknowledged.get())
                    / totInput * 100;
        } else {
            return 0.0;
        }
