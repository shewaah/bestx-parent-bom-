...
import it.softsolutions.bestx.handlers.OrderCancelRequestEventHandler;
...
/**
 * 
 * Purpose: this class manages the state change and the loading of the right handler for the given state
 * 
...
public class CSStrategy implements Strategy, SystemStateSelector {
	 ...
	 * 
	 * @param waitPriceTimeoutMSec
	 *            the new wait price timeout m sec
	 */
	 ...
	 * 
	 * @param curandoRetryTimeout
	 *            the new curando retry timeout
	 */
	...
	public void setLimitFilePdTimes(String limitFilePdTimes) {
		//the final value will be multiplied by 1000 because the timers work with milliseconds intervals
		...
		try {
			...
			LOGGER.warn("Exception while trying to set timers for LFNP orders from string {}.", limitFileNPPriceDiscoveryTimes);
		}
	...
	public OperationEventListener getHandler(Operation operation, OperationState state) throws BestXException {
		...
		// remove multiple quotes handler, it is not needed anymore when state is terminal;
		// in case of new price discovery, it will be removed and recreated, otherwise it would contains quoteid/quotereqid of previous rfqs
		...
		case Cancelled:
			...
			case MARKETAXESS:
				...
				/*
               case BV:
                   handler = new BV_CancelledEventHandler(operation, serialNumberService);
                   break;
				 */               
			default:
				throw new BestXException(Messages.getString("StrategyUnexpectedMarketCode.0", marketCode));
		...
		case OrderNotExecutable:
			handler = new CSOrderNotExecutableEventHandler(operation, serialNumberService, curandoRetryTimeout, marketFinder, operationStateAuditDao, curandoTimerRetriever.getProfile());
			break;
		case Curando: // orders not autoexecutable for size, LF or market, go here
			handler = new CSCurandoEventHandler(operation, serialNumberService, curandoRetryTimeout, curandoTimerRetriever.getProfile());
			break;
		...
		case OrderRejectable:
			...
			break;
		case OrderRevocated:
			handler = new OrderRevocatedEventHandler(operation, serialNumberService, instrumentFinder, operationRegistry);
			break;
		case OrderCancelRequest:
			handler = new OrderCancelRequestEventHandler(operation, serialNumberService, instrumentFinder, operationRegistry);
			break;
		case Rejected: {
			if (marketCode == null) {
		...
		case ManualExecutionWaitingPrice:
			Boolean doNotExecuteMEW = CSConfigurationPropertyLoader.getBooleanProperty(CSConfigurationPropertyLoader.LIMITFILE_DONOTEXECUTE, false);
		...
		case WaitingPrice:
			...
			if (AutoCurandoStatus.SUSPENDED.equalsIgnoreCase(autoCurandoStatus.getAutoCurandoStatus()) || operation.isNotAutoExecute()) {
				doNotExecuteWP = true;
			}
		...
		case LimitFileNoPrice:
			handler = new LimitFileNoPriceEventHandler(operation, serialNumberService, autoCurandoStatus, limitFileNPPriceDiscoveryTimes, marketFinder, operationStateAuditDao);
			break;
		case PriceDiscovery:
			handler = new PriceDiscoveryEventHandler(operation, getPriceService(operation.getOrder()), priceDiscoveryTimeout, priceDiscoveryCustomer, bookDepth, priceDecimals,
					priceDiscoveryConnection);
			break;
	...
	private void checkPreRequisites() throws ObjectNotInitializedException, BestXException {
		...
		/*
		 * if (priceService == null) { throw new ObjectNotInitializedException("Price service not set"); }
		 */
		...
		if (priceDiscoveryCustomer == null) {
			...
			if (priceDiscoveryCustomer == null) {
				throw new ObjectNotInitializedException("Unable to load customer for property PriceDiscovery.Customer. Check your configuration");
			}

		}
		if(curandoTimerRetriever == null) {
			throw new ObjectNotInitializedException("Unable to init CurandoTimerRetriever in CSStrategy instance. Check your configuration");
		}
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see it.softsolutions.bestx.SystemStateSelector#setStateDescription(java.lang.String)
	 */
	@Override
	 ...
	 *            : the codes of the internal market makers
	 */
	 ...
	 *            : automatic curando activation status
	 */
	 ...
	 * 
	 * @param executionDestinationService
	 *            : the new service
	 */
	 ...
	 * 
	 * @param marketCommunicationTimeout
	 *            : timeout in milliseconds
	 */
	 ...
	 * 
	 * @param order
	 *            : the order related to this price service
	 ...
	 * 
	 * @param internalRfqReplyTimeout
	 *            the new internal market exec timeout
	 */
	...
	/**
	 * @param limitFileBestWithinLimitThresholdPDIntervalInSeconds the limitFileBestWithinLimitThresholdPDIntervalInSeconds to set
	 */
	public void setLimitFileBestWithinLimitThresholdPDIntervalInSeconds(long limitFileBestWithinLimitThresholdPDIntervalInSeconds) {
		//the final value will be multiplied by 1000 because the timers work with milliseconds intervals
		if (limitFileBestWithinLimitThresholdPDIntervalInSeconds < LIMIT_FILE_MINIMUM_PD_INTERVAL) {
			LOGGER.warn("Limit file price discovery interval, for orders with best/limit delta within threshold, too short: setting it to a minimum of {} seconds.", LIMIT_FILE_MINIMUM_PD_INTERVAL);
			this.limitFileBestWithinLimitThresholdPDIntervalInSeconds = LIMIT_FILE_MINIMUM_PD_INTERVAL * 1000;
		}
		else {
			this.limitFileBestWithinLimitThresholdPDIntervalInSeconds = limitFileBestWithinLimitThresholdPDIntervalInSeconds * 1000;
		}
	}

	/**
	 * @param limitFileBestOutsideLimitThresholdPDIntervalInSeconds the limitFileBestOutsideLimitThresholdPDIntervalInSeconds to set
	 */
	public void setLimitFileBestOutsideLimitThresholdPDIntervalInSeconds(long limitFileBestOutsideLimitThresholdPDIntervalInSeconds) {
		//the final value will be multiplied by 1000 because the timers work with milliseconds intervals        
		if (limitFileBestOutsideLimitThresholdPDIntervalInSeconds < LIMIT_FILE_MINIMUM_PD_INTERVAL) {
			LOGGER.warn("Limit file price discovery interval, for orders with best/limit delta outside threshold, too short: setting it to a minimum of {} seconds.", LIMIT_FILE_MINIMUM_PD_INTERVAL);
			this.limitFileBestOutsideLimitThresholdPDIntervalInSeconds = LIMIT_FILE_MINIMUM_PD_INTERVAL * 1000;
		}
		else {
			this.limitFileBestOutsideLimitThresholdPDIntervalInSeconds = limitFileBestOutsideLimitThresholdPDIntervalInSeconds * 1000;
		}
	}

	/**
	 * 
	 * @return limitFileBestWithinLimitThresholdPDIntervalInSeconds the limitFileBestWithinLimitThresholdPDIntervalInSeconds
	 */
	...
	/**
	 * 
	 * @return limitFileBestOutsideLimitThresholdPDIntervalInSeconds the limitFileBestOutsideLimitThresholdPDIntervalInSeconds 
	 */
