...
public class EngineControllerImpl extends EngineController {
   ...
   public void init() throws BestXException {
      ...
      try {
         ...
         String[] tableNames = new String[] { "Rfq_Order", "ExecutionReport", "Attempt", "Book", "MarketExecutionReport", "MarketOrder", "MarketSecurityStatus", "Operation", "OperationBinding",
               "OperationState", "PriceTable", "Proposal", "RankingHistory", "TabHistoryOrdini", "TabHistoryStati", "TabOrderFill", "TabTentativi", "TentativiStatoMercato" };
   ...
   public void reloadTimerProfiles() {
      ...
      boolean success = true;
      try {
         curandoTimerRetriever.init();
      }
      catch (Exception e) {
         LOGGER.error("An error occurred while reloading Timer Profiles: {}", e.getMessage(), e);
         success = false;
      }
      LOGGER.info(success ? "Configuration reloaded successfully" : "There were problems while reloading configuration");
   }
   ...
   private void checkPreRequisites() throws ObjectNotInitializedException {
      ...
      if (curandoTimerRetriever == null) {
         throw new ObjectNotInitializedException("curandoTimerRetriever not set");
      }
   }
