 ...
 * 
 **/
public class FormalCurrencyFilter implements OrderValidator {
    private static final Logger LOGGER = LoggerFactory.getLogger(FormalCurrencyFilter.class);
    //	@Override
    /* (non-Javadoc)
    ...
    public OrderResult validateOrder(Operation unused, Order order) {
        ...
        result.setReason("");		
        result.setValid(order.getCurrency() != null);
        if(!result.isValid()) {
            result.setReason(Messages.getString("FormalCurrency.0"));
        } else if(order.getInstrument() != null && order.getInstrument().getCurrency() == null) {
            LOGGER.error("Order rejected, the instrument currency cannot be null");
            result.setValid(false);
            result.setReason(Messages.getString("FormalCurrency.1"));
        }
