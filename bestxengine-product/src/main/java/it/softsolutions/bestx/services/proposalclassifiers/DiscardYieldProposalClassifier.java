 ...
 * 
 */
public class DiscardYieldProposalClassifier implements ProposalClassifier {
	private static final Logger LOGGER = LoggerFactory.getLogger(DiscardYieldProposalClassifier.class);
	...
	public ClassifiedProposal getClassifiedProposal(ClassifiedProposal proposal, Order order, List<Attempt> previousAttempts, Set<Venue> venues, ClassifiedBook book) {
		...
		if (proposal.getPriceType() != null && proposal.getPriceType() == PriceType.YIELD) {
			proposal.setProposalState(Proposal.ProposalState.REJECTED);
			proposal.setReason(Messages.getString("DiscardYield.0"));
