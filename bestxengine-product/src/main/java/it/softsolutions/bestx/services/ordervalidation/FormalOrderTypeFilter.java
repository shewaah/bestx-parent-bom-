 ...
 * 
 **/
public class FormalOrderTypeFilter implements OrderValidator {
    ...
    public OrderResult validateOrder(Operation unused, Order order) {
        ...
        result.setReason("");		
        result.setValid(order.getType() != null
                        && (order.getType().equals(Order.OrderType.MARKET) || order.getType().equals(Order.OrderType.LIMIT)));
        if(!result.isValid()) {
            result.setReason(Messages.getString("FormalOrderType.0"));
        }
        return result;
    }
