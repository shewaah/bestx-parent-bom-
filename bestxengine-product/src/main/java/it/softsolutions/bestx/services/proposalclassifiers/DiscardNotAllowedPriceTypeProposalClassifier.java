/*
* Copyright 1997-2023 SoftSolutions! srl 
* All Rights Reserved. 
* 
...
/**  
*
* Purpose: this class is mainly for discard all proposals with not allowed price types 
*
...
* 
**/
public class DiscardNotAllowedPriceTypeProposalClassifier implements ProposalClassifier {
   private static final Logger LOGGER = LoggerFactory.getLogger(DiscardNotAllowedPriceTypeProposalClassifier.class);

   private Set<String> allowedPriceTypesSet = new HashSet<>();

   public void setAllowedPriceTypes(String allowedPriceTypes) {
      this.allowedPriceTypesSet = new HashSet<>();
      if (StringUtils.isNotBlank(allowedPriceTypes)) {
         this.allowedPriceTypesSet.addAll(Arrays.asList(allowedPriceTypes.split(",")));
      }
   }
   ...
   public ClassifiedProposal getClassifiedProposal(ClassifiedProposal proposal, Order order, List<Attempt> previousAttempts, Set<Venue> venues, ClassifiedBook book) {
      if (proposal.getPriceType() != null && !allowedPriceTypesSet.contains(proposal.getPriceType().name())) {
         proposal.setProposalState(Proposal.ProposalState.REJECTED);
         proposal.setReason(Messages.getString("DiscardPriceType.0", proposal.getPriceType().name()));
