 ...
 * 
 **/
public class FormalISINFilter implements OrderValidator {
    ...
    public OrderResult validateOrder(Operation unused, Order order) {
        ...
        if (order.getInstrument() == null){
            ...
            result.setValid(order.getInstrument().getIsin()!=null
                            && order.getInstrument().getIsin().length()==12);
        }
        if(!result.isValid()) {
            result.setReason(Messages.getString("FormalISIN.0", order.getInstrument()));
        }
        return result;
    }
