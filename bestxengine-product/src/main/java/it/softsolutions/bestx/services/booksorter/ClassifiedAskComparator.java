...
/**  
 *
 * Purpose: comparator to sort ask side ClassifiedProposals in a Book
 *
 ...
 * 
 **/
public class ClassifiedAskComparator implements Comparator<ClassifiedProposal> {

	private static final Logger LOGGER = LoggerFactory.getLogger(ClassifiedAskComparator.class);
	...
	public int compare(ClassifiedProposal o1, ClassifiedProposal o2) {
		...
		//Order by price
		if (result == 0) { 
			result = o1.getPrice().getAmount().compareTo(o2.getPrice().getAmount());			
		}
		...
		if (result==0) {
			if (o1.getMarket().getRanking() < o2.getMarket().getRanking()) {
				result = -1;
			} else if (o1.getMarket().getRanking() > o2.getMarket().getRanking()) {
				result = 1;
			}
		}
