...
/**
 * 
 * Case 1. (old Client API InfoBus Implementation): BloombergConnector > BloombergToBLPTranslator > BLPMarket Case 2. (new Managed BPipe
 * TradeStac Implementation): BLPConnector > BLPMarket
 * 
...
//                	}
    ...
    public void onExecutionReport(String sessionId, String clOrdID, ExecType execType, OrdStatus ordStatus, BigDecimal accruedInterestAmount, BigDecimal accruedInterestRate, BigDecimal lastPrice,
        ...
        // ClordID : TSOX returns the ClOrdID sent by BestX in QuoteResponse
        try {
            ...
            executor.execute(new OnExecutionReportRunnable(operation, this, market, clOrdID, execType, ordStatus, accruedInterestAmount, accruedInterestRate, lastPrice, contractNo, futSettDate,
                    transactTime, text));
    ...
    public void timerExpired(String jobName, String groupName) {
        ...
        // if (!timerHandle.getIdentifier().contains(SimpleMarketProposalAggregator.TIMER_NAME_SEPARATOR) &&
        // !timerHandle.getIdentifier().contains(getMarketCode().toString())) {
        // LOGGER.info("Timeout reached on Bloomberg price retrieval but identifier was not BBG: " + timerHandle.getIdentifier());
        // return;
        // }
        //
        // SimpleMarketProposalAggregator.getInstance().onTimerExpired(this, timerHandle);
        //
        // String timerName = timerHandle.getIdentifier();
        // marketStatistics.timerExpired();
        // if (timerName != null && timerName.contains(SimpleMarketProposalAggregator.TIMER_NAME_SEPARATOR)) {
        // marketStatistics.pricesResponseReceived(timerName.split(SimpleMarketProposalAggregator.TIMER_NAME_SEPARATOR)[1]);
        // }
    }
